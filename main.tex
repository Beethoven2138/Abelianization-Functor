...
\end{proof}
\begin{remark}
Note, that when $\mathcal{A}$ is infinite, $\prod_{\alpha\in\mathcal{A}}S_\alpha$ is only non-empty if we assume the axiom of choice, since the definition requires us to pick an element from infinitely many sets.
\end{remark}
\begin{theorem}
